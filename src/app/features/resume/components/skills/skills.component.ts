						...
						<ul class="space-y-3">
							@for (skill of group.skills; track skill.name) {
								<li>
									...
									<div class="h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700" role="progressbar" [attr.aria-valuenow]="skill.level" aria-valuemin="0" aria-valuemax="100" [attr.aria-label]="skill.name">
										<div class="h-full rounded-full bg-gradient-to-r from-blue-500 to-blue-700 transition-all duration-700" [style.width.%]="skill.level"></div>
									</div>
								</li>
							}
						</ul>
