				...
				<div class="flex items-center gap-1">
					<ul class="hidden items-center gap-1 sm:flex">
						@for (item of navItems; track item.anchor) {
							<li>
								<a [href]="'#' + item.anchor" class="rounded-md px-3 py-1.5 text-sm font-medium text-slate-600 transition hover:bg-slate-100 hover:text-blue-600 dark:text-slate-300 dark:hover:bg-slate-800">
									{{ item.label }}
								</a>
							</li>
						}
